...
/*   Updated: 2025/10/19 15:50:59 by eraad            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	validate_and_mark_output_target(t_data *data, t_token *current)
{
	t_token	*next;
	...
	if (!next)
		return (data->exit_status = 2, report_error3("syntax error ",
				"near unexpected token `newline", "'"), EXIT_FAILURE);
	if (next->type == REDIR_IN || next->type == REDIR_OUT
		|| next->type == REDIR_APPEND || next->type == HEREDOC
		|| next->type == PIPE)
		return (report_error_token_type(data, next->type), EXIT_FAILURE);
...
t_token	*classify_output_redirections(t_data *data, t_token *tokens)
{
	...
	while (cur)
	{
		if (cur->type == REDIR_OUT || cur->type == REDIR_APPEND)
			if (validate_and_mark_output_target(data, cur) == EXIT_FAILURE)
				return (NULL);
		cur = cur->next;
	}
	return (tokens);
