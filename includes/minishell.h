...
/*   Updated: 2025/10/19 17:50:58 by eraad            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void					report_error3(const char *m1, const char *m2,
							const char *m3);
...
char					*expand_variable(t_data *data, char *variable,
							size_t *i, char *accumulated_line);
char					*expand_segment(t_data *data, char *line, size_t *i,
							char *accumulated_line);
char					*append_current_char(t_data *data, char *line,
							size_t *i, char *accumulated_line);

//*------------------------------ LEXING ------------------------------------*

int						lexer(t_data *data);
int						add_operator_token(t_data *data, char operator, int
							*command_boundary);

int						validate_pipe_syntax(t_data *data);
t_token					*classify_input_redirections(t_data *data,
							t_token *tokens);
t_token					*classify_output_redirections(t_data *data,
							t_token *tokens);
...
int						handle_single_quoted(t_data *data, t_quote *qs,
							char **tokbuf);
int						handle_double_quoted(t_data *data, t_quote *qs,
							char **tokbuf);
int						handle_no_quote(t_data *data, t_quote *qs,
							char **tokbuf, int *command_boundary);
...
t_token					*classify_heredoc_delimiters(t_data *data,
							t_token *tokens);
t_token					*classify_input_redirections(t_data *data,
							t_token *tokens);
...
int						wait_and_cleanup_pipeline(t_data *data, pid_t *pids,
							int count, int *fds);
...
pid_t					launch_builtin_child(t_data *data, t_command *node,
							int *fds, int index);
...
void					handle_export_arg(t_data *data, char *key, char *arg,
							int key_idx);
void					export_update_value(t_data *data, const char *key,
							const char *arg);
...
void					parent_close_after_fork(int *fds, int index,
							int n_cmds);
int						handle_external_command(t_data *data, int *fds,
							int index, pid_t *pid);

t_bool					command_path_is_valid(t_data *data, t_command *node,
							char **command_path);
void					report_command_resolution_error(t_data *data,
							t_command *node);
